...
For the subgraph $G_2$, they show a standard superstring corresponds to the ordering of vertices $v_1,v_2, \ldots,v_n$.  For $H$, they assume that there is an edge from vertex $v_i$ to $v_j$ such that there is a weight of (3) on the edge from the last p string to the next connector $v_{i+1}\#v'_{i+1}$.  Now suppose it takes adding k edges to $H$ to get a Hamiltonian path v_1, v_2, ... , v_{n-1}, v_n.  Then they are able to construct a corresponding Standard superstring, of length $\sum_{i=1}^n (2 + 2d_i + 1) + k + 3 = 2n + 2m + k$ where $d_i$ is the out-degree of $v_i$. 
